//! Modular shift-by-k (left, its gate-level right inverse, and the knob).
use super::*;
...
mod tests {
    ...
    #[test]
    fn mod_shift_left_is_x_times_2k_mod_p() {
        let p = SECP256K1_P;
        const K: usize = 3;
        let mut b = B::new();
        ...
        let outputs: std::collections::HashSet<u64> = v
            ...
            .chain(spill.iter())
            .map(|q| q.0)
            .chain([ovf.0, flag_inv.0])
            .collect();
        let mut seed = Shake128::default();
        seed.update(b"mod_shift_left");
        let mut xof = seed.finalize_xof();
        let mut sim = Simulator::new(nq, nb, &mut xof);
        let xs: Vec<U256> = (0..64).map(|s| rand_lt_p(s as u64 + 100)).collect();
        for s in 0..64 {
        ...
        for s in 0..64 {
            ...
            assert_eq!(get256(&sim, &v, s), exp, "v != x·2^{K} mod p, shot {s}");
        }
        // Everything except v + the returned byproduct registers is clean |0>.
