...
| `verify/mod_fast_reduction_emitted.py` | The **scored hot-path `_fast` modular wrappers** — `mod_add_qq_fast` / `mod_sub_qq_fast` / `mod_double_inplace_fast` (the 58 fast calls, ADR 0027) — proved over their **emitted gates** (ADR 0032, referee F2). Extends the ADR 0031 pattern from plain `mod_add_qq` to the wrappers the circuit actually runs; because these fold the Solinas reduction around the **measurement-based** adder, their streams carry `HMR`+`cz_if`, so the reduction is proved **phase-clean in context**. Replays the real op-streams (dumped by `src/point_add/modfast_dump.rs` at the default/scored config into `mod_fast_ops.json`; drift-guarded) through `proof_toolkit`, outcomes **free/∀**: `add`/`sub` = canonical `(acc±a) mod p` (`a` preserved), `double` = congruence `v'≡2v (mod p)`, `v'<2ⁿ`; flag+ancilla → \|0⟩; phase 0. **Finding:** `mod_double_inplace_fast` does a single fold, so it leaves `v'` unreduced (`∈[p,2ⁿ)`) on a ~2³¹-wide window near `2²⁵⁵` — congruent but not canonical; the 64-shot unit test never samples it. Heavy (`just mod-fast-emitted`, not in the default suite). **Note (ADR 0035):** these bind *reusable reference* primitives (`arith/modular/*_fast`); the scored `ops.bin` is `trailmix_ludicrous`, whose own adder is proved by `scored_add_emitted.py` below. |
| `verify/scored_add_emitted.py` | The **scored circuit's adder** (`trailmix_ludicrous::hybrid_add_adaptive`, the adder the scored square drives) proved over its **emitted gates** (ADR 0036) — the first proof bound to gates the scored `ops.bin` actually runs (ADR 0035's next step). Replays the real op-streams (dumped by `src/point_add/trailmix_ludicrous/scored_add_dump.rs` into `scored_add_ops.json`; drift-guarded) through `proof_toolkit`, outcomes **free/∀**, proving `a'=(a+b) mod 2ⁿ`, `b` preserved, ancilla → \|0⟩, and **phase 0** — at 9 configs spanning **both** dispatch branches (plain + sqrt(n)-chunked, incl. the `PUSH/POP_CONDITION` + `NEG`/`Z` measurement-vented carry uncompute) up to width 256. Light (~5 s; in `just analysis`). |
| `verify/peephole_identities.py` | z3 proofs of the constprop CCX identities, the ripple-carry adder recurrence, the borrow-chain comparator, and the **constprop affine-form soundness premise** (ADR 0033 — XOR-linearity of `eval`, equal/complement/constant affine forms ⇒ equal/opposite/constant values; the real `xor_set` is bound to symmetric-difference-over-canonical-form by an exhaustive `#[cfg(test)]` test in `constprop.rs`; equal/complement/constant proved at the **production 512-variable** universe, linearity via a width-independent per-position atom) — **49 lemmas**. |
